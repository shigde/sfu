...
func (r *Resolver) GetResolvedActorFromActorProperty(actor vocab.ActivityStreamsActorProperty) (models.Actor, error) {
	...
	if !actor.Empty() && actor.Len() > 0 && actor.At(0) != nil {
		...
		apa, e := models.MakeActorFromExernalAPEntity(actorEntity)
		if apa != nil {
			apActor = *apa
			resolved = true
		}
...
func (r *Resolver) GetResolvedActorFromIRI(personOrServiceIRI string) (models.Actor, error) {
	...
	resolved := false
	personCallback := func(c context.Context, person vocab.ActivityStreamsPerson) error {
		apa, e := models.MakeActorFromExernalAPEntity(person)
		if apa != nil {
			apActor = *apa
			resolved = true
		}
	...
	serviceCallback := func(c context.Context, service vocab.ActivityStreamsService) error {
		apa, e := models.MakeActorFromExernalAPEntity(service)
		if apa != nil {
			apActor = *apa
			resolved = true
		}
	...
	applicationCallback := func(c context.Context, app vocab.ActivityStreamsApplication) error {
		apa, e := models.MakeActorFromExernalAPEntity(app)
		if apa != nil {
			apActor = *apa
			resolved = true
		}
